...
class ModelOrders():
    ...
    @classmethod
    def get_orders_pending_db(self, db):
        try:
            cursor = db.connection.cursor()
            sql = "SELECT c.numberTable, f.nameFood, o.quantity, o.descriptionOrd, DATE_FORMAT(o.dateDay, '%Y-%m-%d %H:%i:%s') as formatted_date, o.total, o.served FROM orders o INNER JOIN foodmenu f ON o.idFood = f.idFood INNER JOIN client c ON c.userCode = o.userCode WHERE o.served <> 1 ORDER BY o.idOrder DESC;"
            cursor.execute(sql)
            rows = cursor.fetchall()
            orders = [] 
